    ...
    public function handle()
    {
        try {
            ...
            foreach ($categories as $category_dev_name) {
                ...
                if (isset($category->id)) $this->saveCategory($category->id);
                elsedd('Can not find category with slug name: ' . $category_dev_name . '. Check the category name in category.php config file. Check if this category is steel exist in sima-land datebase.');
            }
        ...
        if ($pageCount > 1) {
            do {
                ...
                if (count($categories) != 0) {
                    foreach ($categories as $category) {
                        $asArray = (array)$category;
                        if ($asArray ['is_leaf'] == 1) $this->category->create($asArray);
                    }
                }
                $pageCount--;
            } while ($pageCount != 0);
        } else {// store them to mongodb
            if (count($categories) != 0) {
                foreach ($categories as $category) {
                    $asArray = (array)$category;
                ...
                \Illuminate\Support\Facades\Log::info('There is no category');
                $this->comment("There is no category");//    dd('There is no category in last level ');
            }
        }
    }
